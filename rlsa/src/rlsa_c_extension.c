...
/*
 ...
 Parameters : image - Python object representation of the numpy array image
                         ...
                         range - minimum distance that two black pixels must have, otherwise all pixels between them will be marked black
...
PyObject* rlsa_smear_c(PyObject* self, PyObject* args)
{
    ...
    // integer value representing the range below which pixels are marked black
    int range;
    ...
    // if range input by user is less than zero, make it zero
    if( range < 0 )
        range = 0;
...
static PyMethodDef methods[] = {
  { "rlsa", rlsa_smear_c, METH_VARARGS, "performs run-length smearin on the image (horizontal and/or vertical)"},
  { NULL, NULL, 0, NULL }
};

// module definition
static struct PyModuleDef pythonRLSA = {
    PyModuleDef_HEAD_INIT,
    "pythonRLSA",
    "A Python module that caculates the run-length smearing of an image and return it",
    -1,
    methods
};

// create the module
PyMODINIT_FUNC PyInit_pythonRLSA(void) {
  Py_Initialize();
  import_array();
  return PyModule_Create(&pythonRLSA);
}
